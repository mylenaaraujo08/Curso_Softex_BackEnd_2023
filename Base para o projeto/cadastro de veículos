...
function alterarVeiculo() {
  console.log("Alterar um veiculo");
  console.log("------------------------");

  let modeloAltera = readline.question("Qual o modelo do veículo que deseja alterar: ");
  let veiculoEncontrado = null;

  for (const veiculo of veiculos) {
    if (veiculo.Modelo.toLowerCase() === modeloAltera.toLowerCase()) {
  ...
  if (veiculoEncontrado) {
    console.log("Veículo encontrado. Forneça os novos dados:");

    let tipo = readline.question("Novo tipo do veiculo: ");
    let modelo = readline.question("Novo modelo do veiculo: ");
    let cores = readline.question("Novas cores disponiveis (separadas por virgula): ").split(",");
    let preco = readline.questionFloat("Novo preco do veiculo: ");

    veiculoEncontrado.Tipo = tipo;
    veiculoEncontrado.Modelo = modelo;
    veiculoEncontrado.Cor = cores;
    veiculoEncontrado.preco = preco;

    console.log(`Veiculo "${modeloAltera}" alterado com sucesso!`);
  } else {
    console.log(`Nenhum veículo encontrado com o modelo "${modeloAltera}"`);
  }
}

function removerVeiculo() {
  console.log("Remover um veiculo");
  console.log("------------------------");

  let modeloRemove = readline.question("Qual o modelo do veículo que deseja remover: ");
  let indexRemover = -1;

  for (let i = 0; i < veiculos.length; i++) {
    if (veiculos[i].Modelo.toLowerCase() === modeloRemove.toLowerCase()) {
      indexRemover = i;
      break;
    }
  }
  ...
  if (indexRemover !== -1) {
    const modeloRemovido = veiculos[indexRemover].Modelo;
    veiculos.splice(indexRemover, 1);
    console.log(`Veiculo "${modeloRemovido}" removido com sucesso!`);
  } else {
    console.log(`Nenhum veículo encontrado com o modelo "${modeloRemove}"`);
  }
}
